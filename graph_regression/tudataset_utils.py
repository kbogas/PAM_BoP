# Directly taken and modified from
# https://github.com/danielegrattarola/spektral/blob/master/spektral/datasets/tudataset.py
...
def edge_index_to_matrix(edge_index, edge_weight, edge_features=None, shape=None):
    ...
    a = sp.csr_array((reordered[1], reordered[0].T), shape=shape)
...
# HARD FIX OF INDICES SINCE THE DATASET ZIP DOES NOT CONTAING THE INDICES FOR THE SMALL SPLIT ANYMORE
zinc_small_indices = {
    "train": [
        167621,
        29184,
...
class TUDataset:
    ...
    **Arguments**
    ...
    url_clean = "https://www.chrsmrrs.com/graphkerneldatasets"  # "https://raw.githubusercontent.com/nd7141/graph_datasets/master/datasets"

    def __init__(self, name, small=True, **kwargs):
        if name not in self.available_datasets():
            raise ValueError(
                "Unknown dataset {}. See {}.available_datasets() for a complete list of"
                "available datasets.".format(name, self.__name__)
            )
        self.name = name
        self.path = f"./data/TUDataset/{name}"
        self.small = small
        self.node_features_size = 0
        self.node_labels_size = 0
        self.edge_features_size = 0
        self.edge_labels_size = 0
        self.download()

    def download(self):
        # if not self.name in os.listdir("/".join(self.path.split('/')[:-1])):
        print(f"Downloading {self.name} dataset.")
        url = "{}/{}.zip".format(self.url_clean, self.name)
        download_file(url, self.path, self.name + ".zip")

        # Datasets are zipped in a folder: unpack them
        parent = self.path
        subfolder = os.path.join(self.path, self.name)
        for filename in os.listdir(subfolder):
            shutil.move(osp.join(subfolder, filename), osp.join(parent, filename))
        os.rmdir(subfolder)

    def read(self):
        print(f"Parsing raw {self.name} files")
        ...
        # Batch index
        node_batch_index = (
            np.loadtxt(fname_template.format("graph_indicator")).astype(int) - 1
        )
        n_nodes = np.bincount(node_batch_index)
        n_nodes_cum = np.concatenate(([0], np.cumsum(n_nodes)[:-1]))

        # Read edge lists
        edges = np.loadtxt(fname_template.format("A"), delimiter=",").astype(int) - 1
        ...
        if "node_attributes" in available:
            x_attr = np.loadtxt(fname_template.format("node_attributes"), delimiter=",")
            if x_attr.ndim == 1:
                x_attr = x_attr[:, None]
            self.node_features_size = x_attr.shape[-1]
            x_list.append(x_attr)
        # print("attributes")
        if "node_labels" in available:
            x_labs = np.loadtxt(fname_template.format("node_labels"))
            # num_classes = len(np.unique(x_labs))
            num_nodes = len(x_labs)
            from scipy.sparse import csr_array
            from sklearn.preprocessing import OneHotEncoder

            ohe = OneHotEncoder(categories=np.arange(28).reshape(1, -1).tolist())
            x_ohe = ohe.fit_transform(x_labs.reshape(-1, 1))
            # x_ohe = csr_array(np.ones(num_nodes), (np.arange(num_nodes).astype(int), x_labs.astype(int)), dtype=np.int8)
            # x_ohe[:, x_labs.astype(int)] = 1
            x_labs = x_ohe
            self.node_labels_size = x_labs.shape[-1]
            x_list.append(x_labs.todense())
        # print("labels")
        if len(x_list) > 0:
        ...
        if "edge_attributes" in available:
            e_attr = np.loadtxt(fname_template.format("edge_attributes"))
            if e_attr.ndim == 1:
                e_attr = e_attr[:, None]
            e_attr = e_attr[mask]
            self.edge_features_size = e_attr.shape[-1]
            e_list.append(e_attr)
        # print("edge attributes")
        if "edge_labels" in available:
            e_labs = np.loadtxt(fname_template.format("edge_labels"))
            if e_labs.ndim == 1:
                e_labs = e_labs[:, None]
            e_labs = e_labs[mask]
            ohe = OneHotEncoder()
            e_ohe = ohe.fit_transform(e_labs.reshape(-1, 1))
            # e_labs = np.concatenate(
            #     [_normalize(el_[:, None], "ohe") for el_ in e_labs.T], -1
            # )
            e_labs = e_ohe.todense()
            self.edge_labels_size = e_labs.shape[-1]
        ...
        if "graph_attributes" in available:
            labels = np.loadtxt(fname_template.format("graph_attributes"))
        elif "graph_labels" in available:
            labels = np.loadtxt(fname_template.format("graph_labels"))
        ...
        all_graphs = [
            Graph(
                ...
                self.node_labels_size,
                self.edge_features_size,
                self.edge_labels_size,
            )
            for x, a, e, y in zip(x_list, a_list, e_list, labels)
        ]
        if self.small:
            split_str = self.name.split("_")[-1]
            data_idx = zinc_small_indices[split_str]
            all_graphs = [all_graphs[int(index)] for index in data_idx]
        assert len(data_idx) == len(all_graphs)

        # Convert to Graph
        print(f"Successfully loaded {self.name} with {len(all_graphs)} graphs.")
        return all_graphs
...
class Graph:
    """
    Helper class for handling the data
    """
    ...
    def __init__(
        ...
        self.node_labels_size = node_labels_size
        self.edge_features_size = edge_features_size
        self.edge_labels_size = edge_labels_size

        self.node_features = self.nodes[:, : self.node_features_size]
        self.node_labels = self.nodes[:, self.node_features_size :].argmax(axis=1)
        if isinstance(self.node_labels, np.matrix):
            self.node_labels = self.node_labels.A1

        self.edge_features = self.edges[:, : self.edge_features_size]
        self.edge_labels = self.edges[:, self.edge_features_size :].argmax(axis=1) + 1
        if isinstance(self.edge_labels, np.matrix):
            self.edge_labels = self.edge_labels.A1

        self.label = self.label_ohe.argmax()


######################## HARDFIX FOR ZINC WITH THE MISMATCH OF NODE LABELS ###############
### THE NODE LABELS IN TRAIN ARE 28, WHILE THE NODE LABELS IN VALID 19 AND THE NODE LABELS
### IN TEST 18. THIS CREATES A MISMATCH IN THE OHE PROCEDURE ##################
def _normalize(x, norm=None, categories="auto"):
    """
    Apply one-hot encoding or z-score to a list of node features
    """
    if norm == "ohe":
        categories = categories.flatten().tolist()
        fnorm = OneHotEncoder(sparse=False, categories=categories)
    elif norm == "zscore":
...
class PAMs_from_ZINC(BaseEstimator, TransformerMixin):
    def __init__(
        self,
        power=5,
        use_log=False,
    ...
    def fit(self, X, y=None):
        ...
        print(f"Fitting on {len(X)} graphs...")
        for x in tqdm(X):
            nodeid2label = {i: n for i, n in enumerate(x.node_labels)}
            rows, cols = x.adj.nonzero()
            relation_type = x.edge_labels
            values = []
            row_labels = np.vectorize(nodeid2label.get)(rows)
            col_labels = np.vectorize(nodeid2label.get)(cols)
            triples = list(zip(row_labels, relation_type, col_labels))
            values = []
            for row_label, r, col_label in triples:
                try:
                    rel_value = self.triple2prime[(row_label, r, col_label)]
                except KeyError:
                    cur_prime = int(nextprime(cur_prime))
                    self.triple2prime[(row_label, r, col_label)] = cur_prime
                    rel_value = cur_prime
                    if "step" in self.step:
                        step = float(self.step.split("_")[1])
                        cur_prime = cur_prime + step
                    elif "factor" in self.step:
                        factor = float(self.step.split("_")[1])
                        cur_prime = cur_prime * factor
                    else:
                        raise NotImplementedError(f"{self.step} not understood...")
    ...
    def transform(self, X):
        print(f"Transforming {len(X)} graphs...")
        if len(X) == len(self.train_PAMS):
            return self.train_PAMS
        features = []
        for x in tqdm(X):
            ...
            rows_to_keep = []
            cols_to_keep = []
            for (row_label, r, col_label), row_index, col_index in zip(
                triples, rows, cols
            ):
                try:
                    rel_value = self.triple2prime[(row_label, r, col_label)]
    ...
    def get_pam_powers(self, csr_):
        A_big_log = csr_.copy()
        A_big_log.data = np.log(A_big_log.data)
        A_gb_orig = gb.io.from_scipy_sparse(A_big_log)
        power_A = [A_big_log]
        power_A_gb = [A_gb_orig]
        for ii in range(1, self.power):
            previous_gb = power_A_gb[-1].dup()
            if self.eliminate_diagonal:
                previous_gb.setdiag(0)
            updated_power_gb = previous_gb.mxm(A_gb_orig, self.semiring).new()
            power_A.append(gb.io.to_scipy_sparse(updated_power_gb))
            power_A_gb.append(updated_power_gb)
        return power_A


class PAMs_from_AQSL(BaseEstimator, TransformerMixin):
    def __init__(
        self,
    ...
    def __call__(self, data: datasets.dataset_dict.DatasetDict):
        graphs = {}
        self.fit(data["train"])
        graphs["train"] = self.train_graphs
        graphs["val"] = self.transform(data["val"])
        graphs["test"] = self.transform(data["test"])
        return graphs

    def fit(self, X: datasets.arrow_dataset.Dataset, y=None):
        self.triple2prime = {}
        cur_prime = 1

        # print(f"Fitting on {len(X)} graphs...")
        for x in X:
            node_labels = np.array(x["node_feat"]).flatten()
            edges = np.array(x["edge_index"])
            nodeid2label = {i: n for i, n in enumerate(node_labels)}
            rows, cols = edges[0, :], edges[1, :]
            # print(rows, cols, rows.max(), cols.max())
            relation_type = np.array(x["edge_attr"]).flatten()
            ...
            if self.add_symmetric:
                ...
                csr_ = csr_matrix((values, (rows, cols)), shape=(num_nodes, num_nodes))
            power_A = self.get_pam_powers(csr_)
            self.train_graphs.append(power_A)
        return self

    def transform(self, X: datasets.arrow_dataset.Dataset):
        transformed_graphs = []
        ...
        for x in X:
            ...
            if self.add_symmetric:
                ...
                csr_ = csr_matrix(
                    (values, (rows_to_keep, cols_to_keep)), shape=(num_nodes, num_nodes)
                )
            power_A = self.get_pam_powers(csr_)
            #             vals = []

            #             for pam in power_A:
            #                 vals.extend(pam.data.tolist())
            #             features.append(vals)
            transformed_graphs.append(power_A)
    ...
    def get_pam_powers(self, csr_):
        ...
        power_A_gb = [A_gb_orig]
        for ii in range(1, self.power):
            updated_power_gb = power_A_gb[-1].mxm(A_gb_orig, self.semiring).new()
            power_A.append(gb.io.to_scipy_sparse(updated_power_gb))
            power_A_gb.append(updated_power_gb)
        return power_A
...
class BoP(BaseEstimator, TransformerMixin):
    def __init__(
        self,
        min_df=2,
        max_df=0.99,
    ...
    def fit(self, X, y=None):
        ...
        for power in range(self.max_p):
            cur_X = [pams[power] for pams in X]

            paths = [g.data for g in cur_X]
            cur_vector = clone(self.vector)
            if any([len(p) for p in paths]):
                ...
                self.unused_powers.append(power)
                continue
        return self  # scipy.sparse.hstack(all_features)#.toarray()
    ...
    def transform(self, X):
        ...
        for power in range(self.max_p):
            ...
            all_features.append(tr_paths)
            index_to_use += 1
        stacked = sp.hstack(all_features)
